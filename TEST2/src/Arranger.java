...
public class Arranger {
	...
	private static double[][] readFile(String arrangerFileName)
		...
		BufferedReader reader = new BufferedReader(new FileReader(arrangerFileName));

		String line = reader.readLine();

		StringTokenizer st = new StringTokenizer(line,",");

		double[][] array = new double[st.countTokens()][st.countTokens()];
	...
	private void saveMatrix(String fileName) throws IOException {
		...
		for(int i = 0 ; i<= 3 ; i ++){
			for(int j = 0 ; j<= 3 ; j++){
				fileout.print(arrangerMatrix[i][j]+ ",");
			}
			fileout.println();
		}
